console.log("COGE Serre-Chevalier JS – via Cloudflare Worker – rendu identique");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const reservationsUrlBase =
    "https://coge-calendrier.cogechirolles.workers.dev/api/serre?pageSize=100";
  ...
  function airtableToReservations(records) {
    ...
    records.forEach((rec) => {
      ...
      // ⚠️ Sécurité : si jamais la vue n’est pas filtrée, on garde uniquement Serre-Chevalier
      const appart = f["Appartement"];
      if (Array.isArray(appart) && !appart.includes("Serre-Chevalier")) return;
  ...
  (async () => {
    try {
      ...
      console.log(`OK Serre-Chevalier: ${RESA_CACHE.length} réservations / ${vacRecords.length} vacances`);
    } catch (e) {
